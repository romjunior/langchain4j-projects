...
public class FileSystemExample {
    ...
    public static void main(String[] args) throws IOException {
        ...
        final var path = System.getenv("EXAMPLE_PATH");

        log.info("staring MCP config with server-filesystem example");

        //do this to kill subprocess when the java process exits
        try(McpTransport transport = new StdioMcpTransport.Builder()
                .command(List.of("/home/junior/.asdf/program/asdf", "exec", "npx", "@modelcontextprotocol/server-filesystem", path))
                .logEvents(true)
                .build()) {
